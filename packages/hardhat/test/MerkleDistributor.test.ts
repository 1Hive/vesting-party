...
describe("MerkleDistributor", () => {
  ...
  describe("#claim", () => {
    it("fails for empty proof", async () => {
      ...
      await expect(
        distributor.claim(0, addresses[0], 10, [])
    ...
    describe("two account tree", () => {
      ...
      it("cannot claim more than once: 0 and then 1", async () => {
        await distributor.claim(
          ...
          addresses[0],
          100,
          tree.getProof(0, addresses[0], BigNumber.from(100)),
          overrides
        );
        await distributor.claim(
          1,
          addresses[1],
          101,
          tree.getProof(1, addresses[1], BigNumber.from(101)),
          overrides
        );
      ...
      it("cannot claim for address other than proof", async () => {
        const proof0 = tree.getProof(0, addresses[0], BigNumber.from(100));
        await expect(
          distributor.claim(1, addresses[1], 101, proof0, overrides)
        ).to.be.revertedWith("MerkleDistributor: Invalid proof.");
      });
      ...
      it("cannot claim more than proof", async () => {
        const proof0 = tree.getProof(0, addresses[0], BigNumber.from(100));
        await expect(
          distributor.claim(0, addresses[0], 101, proof0, overrides)
        ).to.be.revertedWith("MerkleDistributor: Invalid proof.");
      });
    ...
    describe("larger tree", () => {
      ...
      it("gas second down about 15k [ @skip-on-coverage ]", async () => {
        await distributor.claim(
          ...
          addresses[0],
          1,
          tree.getProof(0, addresses[0], BigNumber.from(1)),
          overrides
        );
        const tx = await distributor.claim(
          1,
    ...
    describe("realistic size tree", () => {
      ...
      it("proof verification works", () => {
        ...
        for (let i = 0; i < NUM_LEAVES; i += NUM_LEAVES / NUM_SAMPLES) {
          const proof = tree
            .getProof(i, addresses[0], BigNumber.from(100))
      ...
      it("no double claims in random distribution", async () => {
        for (
          ...
          await expect(
            distributor.claim(i, addresses[0], 100, proof, overrides)
          ).to.be.revertedWith("MerkleDistributor: Drop already claimed.");
        }
      });
    });
