...
describe("VestingVault", () => {
  ...
  it("should only allow owner to grant", async function () {
    await expect(
      vault.connect(other).addTokenGrant(0, otherAddress, 10, 0, 10, 2)
    ).to.be.revertedWith(ERRORS.OWNER);
  });

  it("should only allow owner to revoke", async function () {
    await vault.addTokenGrant(0, otherAddress, 10, 0, 10, 0);

    await expect(vault.connect(other).revokeTokenGrant(0)).to.be.revertedWith(
      ERRORS.OWNER
    );
  });
  ...
  it("should emit an event on revoke", async function () {
    ...
    await expect(vault.revokeTokenGrant(0))
      .to.emit(vault, EVENTS.REVOKED)
      .withArgs(0, otherAddress, 0, 10);
  });
  ...
  it("should emit an event on claim", async function () {
    ...
    await expect(vault.claimVestedTokens(0))
      .to.emit(vault, EVENTS.CLAIMED)
      .withArgs(otherAddress, 1);
  });

  it("should have an vesting duration greater than cliff", async function () {
    await expect(
      vault.addTokenGrant(0, otherAddress, 10, 0, 900, 1000)
    ).to.be.revertedWith(ERRORS.VEST_DURATION);
  });

  it("should have an amount vesting per day greater than zero", async function () {
    await expect(
      vault.addTokenGrant(0, otherAddress, 10, 0, 1001, 1000)
    ).to.be.revertedWith(ERRORS.VEST_AMOUNT);
  });

  it("should reject transfer outside of allowance", async function () {
    await expect(
      vault.addTokenGrant(0, otherAddress, 1001, 0, 10, 0)
    ).to.be.revertedWith(ERRORS.ALLOWANCE);
  });
  ...
  it("can not add a grant if one already exists", async function () {
    ...
    await expect(
      vault.addTokenGrant(0, otherAddress, 200, 0, 10, 1)
    ).to.be.revertedWith(ERRORS.GRANT_EXIST);
  ...
  it("vests immediately if no cliff", async function () {
    ...
    await expect(vault.claimVestedTokens(0))
      ...
      .withArgs(otherAddress, 1000);
    expect((await token.balanceOf(otherAddress)).toString()).to.equal("1000");
    await expect(vault.claimVestedTokens(0)).to.be.revertedWith(
      ERRORS.FULLY_VESTED
    );
  });
