...
contract VestingVault is Ownable {
    ...
    function addTokenGrant(
        ...
        onlyOwner
    {
        require(tokenGrants[_tokenId].amount == 0, "Grant already exists, must revoke first.");
    ...
    function claimVestedTokens(uint256 _tokenId)
        external
        onlyOwner
    {
        uint16 daysVested;
    ...
    function calculateGrantClaim(uint256 _tokenId) private view returns (uint16, uint256) {
        Grant storage tokenGrant = tokenGrants[_tokenId];
